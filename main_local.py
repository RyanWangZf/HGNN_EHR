...
class HGNN_local(torch.nn.Module):
    ...
    def forward(self, feat, label, dsd_sampler=None):
        ...
        emb_user = self.forward_usu(data)

        emb_dise = self.forward_dsd(data, ts_label)
    ...
    def forward_dsd(self, data, label):
        ...
        # for d-s-d
        emb_s = self.symp_embeds(data["dsd_1"]) # ?, 20, 64
        m_d_s = self.linear_dsd_1_1(emb_s) + \
            self.linear_dsd_1_2(emb_s.mul(torch.unsqueeze(target_emb_d,1)))
        symp_1_hop = data["dsd_1"]
        m_d_s = self._avg_on_real_neighbor(m_d_s, symp_1_hop) # ?, k
        m_d_d = self.linear_dsd_1_1(target_emb_d)
        emb_d_last = self.act_fn(m_d_d + m_d_s)
        return emb_d_last
...
def train(**kwargs):
    ...
    for epoch in range(model_param["num_epoch"]):
        ...
        early_stopper(metric_result["ndcg_5"], gnn, "gnn_local")
