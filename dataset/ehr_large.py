...
class EHR_load:
    ...
    def pre_processing(self):
        prefix = self.prefix
        data_path = os.path.join(prefix, "clinic_case.txt")
        self.raw_df = pd.read_table(data_path, header=None,encoding="utf-8")
        
        # get all diseases
        dise_list = self.raw_df[2].apply(lambda x: x.split("|"))
        dise_list = [dise[0] for dise in dise_list]
        top_dise_list = pd.value_counts(pd.Series(dise_list))[:400]
        top_dise_list.to_excel("disecount.xlsx")
        dise_name_list = ["NA"] + top_dise_list.index.tolist()
        f_id2dise = open(os.path.join(self.prefix,"id2disease.txt"),"w", encoding="utf-8")
        for x in dise_name_list:
            f_id2dise.write(x+"\n")
        f_id2dise.close()
        ...
        symp_list = self.raw_df[3].apply(lambda x:x.split("@@"))
